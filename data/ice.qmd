...
Our [ICE frequently asked data questions](/docs/ice.html#faq) is a good place to start before analyzing the data. We also provide a [codebook](/docs/ice/codebook.html) that describes each data table and the fields within them.
...
reactable::reactable(
  ...
  details = function(index) {
    ...
    cols <- latest_df[index, "column_names", drop = TRUE][[1]]
...
reactable::reactable(
  ...
  details = function(index) {
    # Get the column_names for this row
    cols <- linked_df[index, "column_names", drop = TRUE][[1]]
    if (is.null(cols) || nrow(cols) == 0) {
      return(NULL)
    }
    htmltools::div(
...
reactable::reactable(
  ...
  details = function(index) {
    # Get the column_names for this row
    cols <- df[index, "column_names", drop = TRUE][[1]]
    if (is.null(cols) || nrow(cols) == 0) {
      return(NULL)
    }
    htmltools::div(
