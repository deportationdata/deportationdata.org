...
We include the **raw**, original data from ICE as well as, for the arrests table, a **processed**, cleaned version in multiple formats (Excel, Stata, SPSS, and feather for R and Python). The processed version includes an indicator for whether the record appears to be a duplicate, based on the timing of the arrest. In addition, we provide a tool to **explore** the data and download filtered subsets of interest. View the processed data, filter it (by for example location or time), and download the filtered data. 
