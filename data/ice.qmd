  ...
  Original = colDef(
    ...
    header = function(value) {
      htmltools::tags$span(
        `data-bs-toggle` = "tooltip",
        `data-bs-placement` = "top",
        title = "Download the original, unprocessed data as provided by ICE",
        "Original"
      )
    },
    cell = function(value) {
...
We include the **original** data from ICE and, for arrests, a **processed**, cleaned version ([code](https://github.com/deportationdata/ice/blob/main/code/arrests-latest.R)). The processed data includes an indicator for likely duplicates based on the arrest timing. We also provide a tool to **explore** the data and download filtered subsets of interest (for example, by location or time).
...
[Download all the original files from the latest release (ZIP, 737 MB)](https://ucla.box.com/shared/static/vxucfx08xp43c7hbwvx7go0utkjp8ahz.zip){.btn-action-primary .btn-action .btn .btn-outline-primary .btn-sm role="button"} 
...
tibble::tribble(
  ...
  relocate(Original, .after = Source) |>
  reactable::reactable(
    columns = list(
      # release column
      Release = colDef(
      ...
      Original = col_defs_ice$Original
    ),
    pagination = FALSE
  )
```
