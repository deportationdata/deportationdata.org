...
## Frequently asked data questions {#faq}
...
**How should I cite ICE data from the Deportation Data Project?**
...
**I saw your warning about removals data. How can I get the best picture of deportations?**

The only removals data that we recommend using is the removals data from late June 2025 (covering only the period beginning in January 2025). Unfortunately, we believe the removals data we posted in the early June 2025 release was incomplete, even with respect to the period it covered. We did not post removals data at all in the late February or late July releases due to apparent errors. In each case (again with the exception of the 2025 data from late June 2025), we discovered many fewer removals in the data provided than in other publicly available sources. These concerns lead us to advise caution when performing any analysis of removals.

The most complete, while still incomplete, way to count deportations (removals) in the most recent (late July) release is to add up (1) people with departed dates in the relevant period from the arrests, detentions, detainers, and encounters tables; and (2) people with “removed” as their detention release reason in this period in the detentions table. To focus on removals after ICE arrest and detention, filter to the “Stay Release Reason” of “Removed” in the detentions table (while being careful not to count people more than once, since many people have information across more than one row in the detentions table–see our codebook for more details). Note that this method will still result in an undercount, because there are individuals, particularly among those apprehended at the border, who appear only in the ICE removals table and not in the other tables.
