...
fields_tbl <- fields |> 
  ...
  mutate(
   ...
   select(Name = name_merge, Description = description, Type = type, in_recent)
...
reactable(
  fields_tbl |> filter(in_recent == 1) |> select(-in_recent), 
  pagination = FALSE,
  striped    = TRUE,
  searchable = TRUE,
  filterable = FALSE,
...
```{r}
reactable(
  fields_tbl |> filter(in_recent == 0) |> select(-in_recent), 
  pagination = FALSE,
  striped    = TRUE,
  searchable = TRUE,
  filterable = FALSE,
