...
fields_unused <-
  ...
  select(table, column_name, description)

fields <-
  fields |>
  select(table, column_name = name, description, missing_pct) |>
  mutate(column_name = str_to_lower(column_name))
```

We provide a codebook for the EOIR Case dataset. The codebook is a work in progress; there are many things we do not understand in the data, and some of our educated guesses here may be mistaken. We will continue to update the codebook as we learn more, and we welcome feedback and corrections.
...
reactable(
  fields_tbl |>
    # filter(in_recent == 1) |>
    select(Table, Name, Description, `Missing %`) |> #, Type) |>
    # mutate(Name = Name |>
...
reactable(
  fields_unused |>
    # filter(in_recent == 1) |>
    select(Table = table, Name = column_name, Description = description) |> #, Type) |>
    # mutate(Name = Name |>
    #    str_replace_all(" yes no", " (Yes/No)") |>
    #    str_replace_all(" Yes No", " (Yes/no)")) |>
    arrange(Table, Name),
