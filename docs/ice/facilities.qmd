...
- **ICE detention facilities**: Locations where individuals are held in ICE custody. Includes facilities operated by ICE as well as those operated by private contractors and state and local governments. 
...
detention_facility_df <- arrow::read_feather(
  "https://github.com/deportationdata/ice/raw/refs/heads/main/data/facilities.feather"
)

detention_facility_stats <- arrow::read_feather(
  "https://github.com/deportationdata/ice/raw/refs/heads/main/data/facilities-from-detentions.feather"
)

detention_facility_df <-
  detention_facility_df |>
  left_join(
    detention_facility_stats |>
      select(
  ...
  mutate(name = name |> str_to_title() |> tools::toTitleCase())
...
leaflet() |>
  ...
  addCircleMarkers(
    ...
    radius = ~ sqrt(n_stints_2025) / 11, # Scale the radius based on n_stints
    fillOpacity = 0.5,
    stroke = FALSE,
    weight = 1,
    # color = "white",
...
### ICE ERO field offices and check-in offices (sub-offices)
...
detention_facility_df |>
  ...
  reactable(
    ...
    theme = reactableTheme(
      tableStyle = list(fontSize = "14px"),
      headerStyle = list(backgroundColor = "#F4F6F7"),
      backgroundColor = "#F4F6F7"
    ),
    columns = list(
      `Detention Facility Code` = colDef(
        name = "Facility code",
        minWidth = 120
      ),
      Aor = colDef(
      ...
      `N Individuals 2025` = colDef(
        name = "N individuals (2025)",
        format = colFormat(separators = TRUE)
      )
    ),
    columnGroups = list(
      # add types
      colGroup(name = "Type", columns = c("Type", "Type Detailed")),
      colGroup(
        name = "2025 Detentions",
        columns = c("N Stints 2025", "N Individuals 2025")
      )
    )
  )
```
...
download_df <-
  tribble(
    ...
    "ICE ERO areas of responsibility shapefile (county level)",
    "Geographic areas of responsibility for each ICE ERO field office. Provided as a shapefile of county polygon geometries, in both a ZIP file with the .shp and associated files, and as a Feather file with simple feature geometries.",
    as.Date("2025-09-30"),
    list(
      list(
        "https://github.com/deportationdata/ice/raw/refs/heads/main/data/ice-aor-county-shp.zip",
        "",
        "shp"
      ),
      list(
        "https://github.com/deportationdata/ice/raw/refs/heads/main/data/ice-aor-county-shp.feather",
        "",
        "feather"
      )
    )
